from tagbrewer.tag import checker, generators
import pytest
import pathlib
...
def test_collapse_alleles():
    alleles_dict = {"gene1": {"01": "abc", "02": "aab"},
                    "gene2": {"01": "def", "02": "ddf"}}
    assert checker.collapse_alleles(alleles_dict) == ["gene1", "gene2"]
...
class TestGeneDifferences:
    ...
    def test_extract_gene_list_version_parity(self, resource_location):
        original = checker.extract_gene_list(resource_location, "human", "original", "TRAV")
        extended = checker.extract_gene_list(resource_location, "human", "extended", "TRAV")
    ...
    def test_find_gene_differences(self, resource_location, monkeypatch):
        ...
        differences = checker.find_gene_differences(resource_location, "human", "extended", "TRAV")
